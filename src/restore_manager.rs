...
impl RestoreManager {
    pub async fn restore(&self) {
        let archive_path = self
            ...
            .tar_gz_path
            .clone()
            .expect("Tar GZ Path not found");
        let file_contents = RestoreManager::get_content_from_targz_inner_files(&archive_path).await;
        ...
        match file_contents {
            Ok(contents) => {
                for collection_data in contents {
                    RestoreManager::handle_single_collection(
                        collection_data,
                        &client,
                        BackupManager::get_mongodb_database_name(&conn_string).as_str(),
                    )
                    .await;
                }
            }
        ...
        for entry in archive.entries().unwrap() {
            ...
            if entry.header().entry_type().is_file() {
                ...
                let data = CollectionData {
                    data: contents,
                    name: String::from(
                        entry.path().unwrap().file_stem().unwrap().to_str().unwrap(),
                    ),
                };
